...
//
// Instagram ログインから戻ってきた code を long-lived access token
// (60 日) に交換し、IG ユーザー ID と共に HttpOnly Cookie に保存して
// return_to へリダイレクト。
// ============================================================
...
export default async function handler(req: Request): Promise<Response> {
  ...
  try {
    // 1) code → short-lived access token (POST form to api.instagram.com)
    ...
    const shortToken = tokenJson.access_token || tokenJson.data?.[0]?.access_token;
    ...
    // 3) Cookie 保存 (長期トークン + IG user ID)
    const cookieAttrs = `Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${expiresIn}`;
    const headers = new Headers();
    headers.append('Set-Cookie', `ig_access_token=${longToken}; ${cookieAttrs}`);
    headers.append('Set-Cookie', `ig_account_id=${userId}; ${cookieAttrs}`);
