// ============================================================
// GET /api/threads/status?uid=<匿名ID>
//
...
import { isThreadsConfigured, tokKey, jsonRes, type StoredThreadsToken } from './_shared';
...
export default async function handler(req: Request): Promise<Response> {
  ...
  const configured = isThreadsConfigured() && isUpstashConfigured();
  if (!configured) {
  ...
  try {
    const tok = await kvGetJSON<StoredThreadsToken>(tokKey(uid));
