  ...
  const faqs: Array<{ id: string; q: string; a: string }> = [
    {
      id: 'cancel',
      q: '解約はいつでもできますか?',
      // 2026-08-18: 「1 クリックで解約」は実物と違った（確認→引き止め1枚→理由選択と進む）。
      // 実際の手数に言葉を合わせる。違約金・引き止めの電話が無いのは本当なので残す。
      a: `はい、いつでも${cancelPath}から解約できます。理由の入力は任意で、選ばなくても進めます。違約金や引き止めの電話は一切ありません。解約後も契約期間の末日まではそのまま使えます。`,
    },
    {
      id: 'data',
      q: '解約したら、データは消えますか?',
