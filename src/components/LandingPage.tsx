...
export default function LandingPage({ onEnterApp }: Props) {
  ...
  return (
    <div style={{ background: BG_DARK, color: '#fff', minHeight: '100dvh', fontFamily: '"Inter","游ゴシック","Hiragino Kaku Gothic ProN",sans-serif', overflowX: 'hidden' }}>
      ...
      {/* ── ヘッダ ────────────────────────────── */}
      <header className="lp-safe" style={{ position: 'sticky', top: 0, zIndex: 50, background: 'rgba(10,10,20,0.92)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
    ...
    <div style={{ display: 'flex', gap: 2, background: 'rgba(255,255,255,0.08)', borderRadius: 8, padding: 2 }}>
      {opts.map(o => (
        <button
          ...
          style={{ background: lang === o.key ? 'rgba(255,255,255,0.18)' : 'transparent', color: lang === o.key ? '#fff' : 'rgba(255,255,255,0.5)', border: 'none', borderRadius: 8, padding: '0.45rem 0.75rem', fontSize: '0.78rem', fontWeight: 700, cursor: 'pointer', transition: 'background 0.15s, color 0.15s', minWidth: 44, minHeight: 44, display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}
        >
          {o.label}
        </button>
      ))}
