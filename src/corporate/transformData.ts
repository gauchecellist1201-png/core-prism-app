 ...
 * ここが書かれていないサイトは、決裁の場に持ち込めない。
 */
export const ENGAGEMENT_STEPS: { no: string; title: string; body: string }[] = [
  {
    no: '01',
    title: 'まず、話を聞く',
    body: 'いま何に時間とお金が消えているかを伺います。その場で、AIで変えられる所と変えられない所を切り分けてお返しします。初回のご相談に費用はいただきません。',
  },
  {
    no: '02',
    title: '診断して、1枚にする',
    body: 'どの業務を、どの順番で、いくらの効果を見込んで変えるか。90日・1年・3年で区切ったロードマップにします。ここまでで終わりにしていただいても構いません。',
  },
  {
    no: '03',
    title: '書面で決める',
    body: '範囲・期間・費用・成果物・秘密保持を、着手の前に書面にします。作ったものの権利は、原則としてお客様に帰属します。',
  },
  {
    no: '04',
    title: '二週間ごとに、動くものを見せる',
    body: '完成まで何も見えない時間を作りません。二週間ごとに実際に触れる状態でお見せし、違うと感じた所はその場で向きを変えます。',
  },
  {
    no: '05',
    title: '定着させて、直し続ける',
    body: '使われ方を見て直します。月次で数字を持ち寄り、次の一手を決めます。使われていない機能は、増やさずに止めます。',
  },
];

export const ENGAGEMENT_TERMS: { t: string; d: string }[] = [
  { t: '契約の形', d: '月額でご一緒する伴走（準委任）と、範囲を決めた開発（請負）を、内容に合わせて選びます。' },
  { t: '出る人', d: '代表が、最初のご相談から運用まで通して担当します。担当が入れ替わって話が振り出しに戻ることはありません。' },
  { t: '報告', d: '二週間ごとに動く画面、月次で数字。稟議に要る形の書面が必要であれば、書面でお出しします。' },
  { t: 'やめるとき', d: '伴走契約は1か月前のご連絡で終われます。違約金はいただきません。' },
  { t: '成果物', d: 'ソースコード・設計・アカウントは御社の環境に置きます。当社に預けたままにはしません。' },
  { t: '効かないとき', d: '効果が見込めないと判断したら、こちらから止める提案をします。続けること自体を目的にしません。' },
];

// ── 機密と安全（SECURITY）───────────────────────────
/**
 * 認証・認定は取得していない。取得していないものは書かない。
 * ここに並べるのは「実際にやっている取り扱い」だけにする。
 */
export const SECURITY_ITEMS: { t: string; d: string }[] = [
  {
    t: '預かった情報を、AIの学習に使わせない',
    d: '業務データを渡す先は、入力を学習に使わない契約形態のAPIに限ります。社内文書を無料の対話サービスに貼り付ける運用はしません。',
  },
  {
    t: '秘密保持は、話を始める前に',
    d: 'NDAは打ち合わせの前に締結できます。御社の書式で構いません。',
  },
  {
    t: '鍵を、人とコードから切り離す',
    d: 'APIキーや認証情報はコードにも履歴にも書かず、環境変数として保持します。誰がどのデータに触れられるかは、役割で分けます。',
  },
  {
    t: '誰が何を変えたか、後から追える',
    d: '重要な操作は記録に残します。画面上の見た目だけで「できたこと」にせず、実際の書き込み結果を確認してから完了とします。',
  },
  {
    t: '置き場所と、消し方を先に決める',
    d: 'データをどこに置き、いつ消し、終了時に何をお返しするか。預かる前に契約書へ書きます。',
  },
  {
    t: '人が最後に確かめる所を、決めておく',
    d: '見積・請求・対外文書など、AIの出力をそのまま外へ出さない工程を、はじめに合意します。',
  },
];

// ── よくあるご質問（FAQ）───────────────────────────
export const CORP_FAQ: { q: string; a: string }[] = [
  {
    q: 'AIに詳しい社員がいません。それでも始められますか。',
    a: '始められます。詳しい人がいる前提の話はしません。いま手で回している仕事の中身を伺うところから始めます。使う人が学び直さなくて済むよう、道具の側を業務に合わせます。',
  },
  {
    q: 'まず小さく試したいのですが。',
    a: 'できます。診断だけ、あるいは一業務だけの構築から承ります。効果が見えなければ、そこで止めていただいて構いません。',
  },
  {
    q: 'すでに基幹システムがあります。作り直しになりますか。',
    a: 'いいえ。入れ替えを前提にしません。いまのシステムを残したまま、その周りをAIでつなぐ形から検討します。替えたほうが安くなる場合だけ、根拠を添えてご提案します。',
  },
  {
    q: '作っても、社員が使ってくれないのではないですか。',
    a: 'いちばん多い失敗がそれです。だから「新しい画面を覚えてもらう」設計を避け、いま使っている道具（メール・LINE・表計算）の側にAIを寄せます。それでも使われない機能は、足さずに止めます。',
  },
  {
    q: '費用はどのくらいかかりますか。',
    a: '内容によって変わります。目安は〈ご予算〉に置いていますが、診断のうえで必要なものだけをお見積りします。要らないと判断したものは提案しません。',
  },
  {
    q: 'どのくらいで変わりますか。',
    a: 'ロードマップは 90日・1年・3年 で区切ってお出しします。最初の業務が動き出すまでの期間は範囲によって変わるため、診断の場で具体的にお伝えします。',
  },
  {
    q: '「2026年 設立予定」とありますが、任せて問題ありませんか。',
    a: '法人化は準備中で、いまは個人事業として契約しています。隠さずに書いています。判断の材料として、本番で動いているものを〈プロダクト〉で実際に触っていただけます。契約・秘密保持・成果物の権利については、法人と同じ条件で書面を交わします。',
  },
  {
    q: '遠方でも対応できますか。',
