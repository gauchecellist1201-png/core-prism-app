...
function Showcase() {
  ...
  const daikouMailto = `mailto:info@core-ai.jp?subject=${encodeURIComponent('【Crystal 設置代行】お願いします')}&body=${encodeURIComponent(
    `サイトURL: \nブランド名: ${config.brandName}\nご希望 (あれば): \n\n--- 以下はそのままで大丈夫です (あなたの設定データ) ---\n${pageUrl}`,
  )}`;

  const mailtoCta = (plan: string) =>
    `mailto:info@core-ai.jp?subject=${encodeURIComponent(`【Crystal 導入相談】${plan}`)}&body=${encodeURIComponent(
      `ブランド名: ${config.brandName}\n業種: ${config.industry}\nご希望プラン: ${plan}\nサイトURL: \nご相談内容: `,
    )}`;
        ...
        >
          <div style={{
            ...
            <p style={{ margin: '12px 0 0', fontSize: 11.5, lineHeight: 1.8, color: P.fgSubtle, textAlign: 'center' }}>
              ご不明点はメール1通で:{' '}
              <a href="mailto:info@core-ai.jp" style={{ color: P.gold, fontWeight: 700, textDecoration: 'none', whiteSpace: 'nowrap' }}>info@core-ai.jp</a>{' '}
              — 24時間以内にご返信します。
            </p>
          </div>
        </div>
          ...
          <div style={{ flex: '1 1 320px', minWidth: 0, display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div id="method-daikou" style={{
              ...
              <p style={{ margin: '0 0 12px', fontSize: 12.5, lineHeight: 1.8, color: P.fgMuted }}>
                ...
                メールが開かない場合は、上の専用リンクをコピーして info@core-ai.jp へ送ってください。
              </p>
              <a
                href={daikouMailto}
                style={{
