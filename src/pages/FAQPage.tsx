...
const FAQS: FaqItem[] = [
    ...
    a: (
      <>
        ...
        <p>
          ...
          「自分が使いたくないもの」は出さない、という作り方をしています。
        </p>
        <p>
  ...
  },
  {
    ...
    a: (
      <>
        <p>
          正直にお答えします。<strong>「絶対に続きます」とは約束できません。</strong>
          どんな会社でも同じですが、小さく始めているぶん、そこは隠さずお伝えします。
        </p>
        <p>だからこそ、こういう作りにしています。</p>
        <ul>
          <li><strong>お試しはカード登録なし</strong>で {TRIAL_BASE_DAYS} 日間。合わなければ、何もせず離れられます</li>
  ...
  },
  {
    ...
    a: (
      <>
        ...
        <p>
          「導入 ◯◯ 社」「効果 ◯◯ %」といった数字は、
          <strong>本当に出るまでこのページに書きません</strong>。出せるようになったらここに書きます。
        </p>
        <p>
    ...
    a: (
      <>
        <p>
          ...
          ひらけたかどうかを測っています。書いてあるのは、その実測の結果だけです。
        </p>
        <p>
  ...
  },
  {
    ...
    a: (
      <>
        <p>
          <strong>アプリの中に「領収書を発行する」画面は、まだありません。</strong>（作っていない機能をあるように書きたくないので、正直に書いています）
        </p>
        <p>
  ...
  return (
    ...
    >
      <p>
        ...
        <strong>実物で確認できたことだけを書いています。</strong>まだ無いものは「まだ無い」と書いています。
      </p>
      <p>
